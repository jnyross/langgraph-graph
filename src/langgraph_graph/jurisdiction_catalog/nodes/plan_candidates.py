...
DEFAULT_SEED = Path(__file__).resolve().parents[4] / "data/jurisdictions/jurisdiction_catalog_seed.json"
...
def plan_candidates(state: dict[str, Any]) -> dict[str, Any]:
    path = Path(state.get("seed_path") or DEFAULT_SEED)
    try:
        entries = _read(path)
        current = load_catalog()
    except Exception as exc:
        return {"candidates": [], "error": f"candidate planning failed: {exc}", "errors": [str(exc)]}
    by_id: dict[str, Candidate] = {}
    wanted = set(state.get("levels") or [])
    regions = {slugify(x) for x in (state.get("regions") or [])}
    for raw, source in [(x, "seed") for x in entries] + [
        ...
        try:
            ...
            item = Candidate(
                id=cid, name=name, level=level, parent_id=parent,
                domains_priority=list(raw.get("domains_priority") or ["all"]),
                rationale=str(raw.get("rationale") or ""), source=source,
            )
        except Exception:
            continue
        ...
        if prior is None or (prior.source == "current_catalog" and source == "seed"):
            by_id[item.id] = item
    return {"candidates": [by_id[k] for k in sorted(by_id)], "error": None}
